/** Isolated browser QA; no production API, installs, roles or model execution. */
import { mkdtempSync } from "node:fs";
import { tmpdir } from "node:os";
import path from "node:path";
import { defineConfig } from "@playwright/test";
const port = 32062, baseURL = `http://127.0.0.1:${port}`;
export default defineConfig({
  testDir: ".", testMatch: "ml-use-runs.visual.ts", outputDir: mkdtempSync(path.join(tmpdir(), "sclib-ml-runs-browser-artifacts-")),
  ...
  webServer: { command: `${JSON.stringify(process.execPath)} ${JSON.stringify(path.join(__dirname, "scientific-review-server.cjs"))}`,
    url: `${baseURL}/dashboard/research/ml-runs`, reuseExistingServer: false, timeout: 90000,
