/** Isolated browser QA; synthetic HTTP only, no production or provider calls. */
import { mkdtempSync } from "node:fs";
import { tmpdir } from "node:os";
import path from "node:path";
import { defineConfig } from "@playwright/test";

const port = 32038, baseURL = `http://127.0.0.1:${port}`;
export default defineConfig({
  testDir: ".", testMatch: "answer-history.visual.ts",
  outputDir: mkdtempSync(path.join(tmpdir(), "sclib-answer-history-browser-")),
  ...
  webServer: {
    ...
    url: `${baseURL}/dashboard/history`, reuseExistingServer: false, timeout: 90_000,
