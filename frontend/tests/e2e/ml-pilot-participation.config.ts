/** Isolated browser QA; no production API, installs, real roles or scientific reviews. */
import { mkdtempSync } from "node:fs";
import { tmpdir } from "node:os";
import path from "node:path";
import { defineConfig } from "@playwright/test";
const port = 32064, baseURL = `http://127.0.0.1:${port}`;
export default defineConfig({
  testDir: ".", testMatch: "ml-pilot-participation.visual.ts", outputDir: mkdtempSync(path.join(tmpdir(), "sclib-ml-pilot-browser-artifacts-")),
  ...
  webServer: { command: `${JSON.stringify(process.execPath)} ${JSON.stringify(path.join(__dirname, "scientific-review-server.cjs"))}`,
    url: `${baseURL}/dashboard/research/ml-pilots`, reuseExistingServer: false, timeout: 90000,
