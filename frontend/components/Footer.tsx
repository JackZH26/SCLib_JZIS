...
export async function Footer() {
  ...
  return (
    <footer className="mt-16 border-t border-sage-border bg-sage-surface/60">
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
          ...
          {groups.map((group) => (
            <div key={group.title}>
              ...
              <ul className="mt-3 space-y-2 text-sm">
                {group.links.map(([href, label]) => <li key={href}><Link href={href} className="text-sage-muted hover:underline">{label}</Link></li>)}
              </ul>
            </div>
          ))}
        </nav>
        <div className="mt-9 border-t border-sage-border pt-5 text-xs text-sage-muted">
          ...
          {(siteVersion || version?.dataset_version) && <p className="mt-3 flex flex-wrap gap-x-4 gap-y-1" aria-label="Library version">
