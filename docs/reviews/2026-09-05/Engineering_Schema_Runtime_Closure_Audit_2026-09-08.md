...
| Full batch-32 API run | Still running when this audit was finalized; it began before the final dossier fan-out fix. Parent separately reported 122 focused API tests passed after that fix. Subsequent outcome: 5,033 passed, 1 failed, 3 opt-in skips, 20 existing warnings in 2,059.24 s. The new queue test assumed its own material was the first global result; an earlier valid MgB2 record violated that assumption. Batch 33 corrects the identity-scoped test and records its current-source regression separately. This is not a passing full post-fix run. |
