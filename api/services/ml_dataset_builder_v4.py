"""Label-currentness-aware private datasets without rewriting v1/v2/v3 artifacts.

A complete separately pinned label observation adds only negative gates before
base-label construction. The full frozen graph still determines leakage groups;
all nested cohorts, splits and train-only transforms are rebuilt from raw inputs.
Neither source observations nor review declarations grant scientific authority.
"""
...
from services.ml_dataset_builder_v3 import (
    _implementation as _v3_implementation,
)
...
def _implementation():
    result = _v3_implementation()
    result["version"] = VERSION
    for package, names in {
        "models": ["ml_task_v4"],
        "services": ["ml_dataset_builder_v4", "ml_label_companion", "ml_label_observation"],
    }.items():
        for name in names:
...
def _check_label_receipt(
    label, index, dataset_id, manifest_hash, source_hash, review_hash, label_hash
):
    """Require a complete exact root-label observation, including no-input rows."""
    from services.ml_label_companion import LABEL_HOLD_CODES
    ...
    require(
        ...
        and label["version"] == "ml-label-companion/1.0.0"
        and label["observation_semantics"] == "captured_not_live"
        and label["base_manifest_sha256"] == manifest_hash
        and label["source_companion_sha256"] == source_hash
        and label["review_companion_sha256"] == review_hash
        and label["label_companion_sha256"] == label_hash,
        "label receipt independent pins mismatch",
    )
    require(
        type(label["observation_sha256"]) is str
        and re.fullmatch("[0-9a-f]{64}", label["observation_sha256"]),
        "label observation pin required",
    )
    require(
        type(label["authority"]) is dict
        and set(label["authority"])
        == {
            "scientific_acceptance",
            "ml_training_approved",
            "public_release_authorized",
        ...
        and all(value is False for value in label["authority"].values()),
        "label receipt authority invalid",
    )
    examples = {
        identifier: row["data"]["claim_id"]
        for (table, identifier), row in index.items()
        if table == "ml_examples" and row["data"]["dataset_snapshot_id"] == dataset_id
    }
    claims = set(examples.values())
    for key, table, expected in (
        ("example_pins", "ml_examples", set(examples)),
        ("claim_pins", "material_claims", claims),
    ):
        pins = label[key]
        require(
            type(pins) is dict and set(pins) == expected,
            "label receipt complete root inventory required",
        )
        for identifier, pin in pins.items():
            require(
                type(identifier) is str
                ...
                and (table, identifier) in index
                and pin["row_sha256"] == index[(table, identifier)]["row_sha256"],
                "label receipt frozen row pin mismatch",
            )
    require(
        type(label["claim_holds"]) is dict and set(label["claim_holds"]) == claims,
        "label receipt complete hold inventory required",
    )
    for codes in label["claim_holds"].values():
        require(
            type(codes) is list
            and len(codes) <= len(LABEL_HOLD_CODES)
            and all(type(code) is str and code in LABEL_HOLD_CODES for code in codes)
            and codes == sorted(set(codes)),
            "label receipt reason codes invalid",
        )
...
def build_task_dataset_v4(
    ...
    *,
    ...
    task = validate_task_v4(task)
    _pin(task, expected_task_sha256, "v4 task")
    ...
    label = verify_ml_label_companion(
        label_companion,
        base_manifest=manifest,
        expected_base_manifest_sha256=expected_manifest_sha256,
        source_companion=companion,
    ...
    for example in examples:
        ...
        label_codes = label["claim_holds"][example["claim_id"]]
        if label_codes:
            reasons.update(label_codes)
            reasons.add("label_currentness_held")
        ...
        candidate = {
            ...
            "captured_label_currentness": {
                "observation_sha256": label["observation_sha256"],
                "example_row_sha256": label["example_pins"][example["id"]]["row_sha256"],
                "claim_row_sha256": label["claim_pins"][example["claim_id"]]["row_sha256"],
                "reason_codes": list(label_codes),
            },
        }
    ...
    bundle = {
        ...
        "input_pins": {
            ...
            "label_observation_sha256": label["observation_sha256"],
        },
        "review_observation": {
        ...
        "label_observation": {
            "scope": "captured_at_pinned_observation_not_current_authorization",
            "policy": task["label_currentness_policy"],
            "observation_sha256": label["observation_sha256"],
            "example_count": len(label["example_pins"]),
            "claim_count": len(label["claim_pins"]),
            "held_claim_count": sum(bool(codes) for codes in label["claim_holds"].values()),
            "held_example_count": sum(
                bool(label["claim_holds"][example["claim_id"]]) for example in examples
            ),
            "authority": dict(AUTHORITY),
        },
        ...
        "split_report": {
            ...
            "assignment_scope": "rebuilt_after_label_gates_from_complete_frozen_components",
            "cross_artifact_assignment_stability": "not_guaranteed",
        },
        ...
        "data_card": {
            ...
            "label_currentness_scope": "captured negative-only root-label holds exclude B and every nested view before splitting and fitting; empty holds never establish scientific eligibility",
            "cross_artifact_comparisons": "label exclusions may change surviving split assignments; only within-artifact same-cohort comparisons are established",
...
def verify_task_dataset_v4(bundle, *, expected_bundle_sha256, **inputs):
    _pin(bundle, expected_bundle_sha256, "compiled v4 bundle")
    recomputed = build_task_dataset_v4(**inputs)
    require(canonical(bundle) == canonical(recomputed), "compiled v4 bundle recomputation mismatch")
    return {
        "version": VERSION,
        "bundle_sha256": expected_bundle_sha256,
        "integrity_verified": True,
        "technical_gate": recomputed["gate"]["status"],
        **{
            name: recomputed["input_pins"][name]
            for name in (
                "review_companion_sha256",
