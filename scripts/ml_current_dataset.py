#!/usr/bin/env python3
"""Offline v4 dataset build/verify with independently pinned label observations.

Use the locked API interpreter. Captured negative-only observations are not live
authorization, scientific acceptance, or permission to train or redistribute.
This entry point performs no database, network, provider, or publication calls.
"""
...
from services.ml_label_companion import MAX_BYTES as MAX_LABEL_BYTES
...
def read_label_companion(path, expected):
    """Capture bounded canonical private bytes and their exact file identity."""
    require(type(expected) is str and HASH.fullmatch(expected), "independent label file pin required")
    path = Path(path)
    require(path.name.endswith(".json"), "JSON label companion required")
    directory = _directory_fd(path.parent)
    try:
        descriptor = os.open(path.name, os.O_RDONLY | os.O_NOFOLLOW | os.O_NONBLOCK, dir_fd=directory)
        try:
            before = os.fstat(descriptor)
            require(stat.S_ISREG(before.st_mode) and before.st_nlink == 1 and before.st_size <= MAX_LABEL_BYTES,
                    "label companion must be bounded regular file without aliases")
            chunks, size = [], 0
            while True:
                chunk = os.read(descriptor, min(1024 * 1024, MAX_LABEL_BYTES - size + 1))
                if not chunk:
                    break
                size += len(chunk)
                require(size <= MAX_LABEL_BYTES, "label companion byte limit")
                chunks.append(chunk)
            after = os.fstat(descriptor)
            require(before.st_size == size and _signature(before) == _signature(after),
                    "label companion changed during read")
        finally:
            os.close(descriptor)
    finally:
        os.close(directory)
    payload = b"".join(chunks)
    require(hashlib.sha256(payload).hexdigest() == expected, "label companion file pin mismatch")
    value = strict_json(payload)
    require(canonical(value) == payload, "canonical label companion required")
...
def run(*, mode, manifest_path, expected_manifest_sha256, companion_path, expected_companion_sha256,
    ...
    """Re-read every independently pinned input before returning/writing output."""
    ...
    if mode == "build":
        bundle = build_task_dataset_v4(**inputs)
        report = {
            "version": bundle["version"], "technical_gate": bundle["gate"]["status"],
            "gate_reason_codes": bundle["gate"]["reason_codes"], "no_go_views": bundle["gate"]["no_go_views"],
            "coverage": bundle["coverage"], "bundle_sha256": digest(bundle),
            "review_observation_sha256": bundle["input_pins"]["review_observation_sha256"],
            "label_observation_sha256": bundle["input_pins"]["label_observation_sha256"],
            "review_scope": "captured_observation_not_current_authorization",
            "label_currentness_scope": "captured_negative_only_not_live_authorization",
            **bundle["authority"],
        }
    else:
        bundle, bundle_signature = read_json(bundle_path, expected_bundle_sha256)
        report = verify_task_dataset_v4(bundle, **inputs, expected_bundle_sha256=expected_bundle_sha256)
        require(read_json(bundle_path, expected_bundle_sha256) == (bundle, bundle_signature),
