...
class SchemaLifecycleBoundaryTests(unittest.TestCase):
    ...
    def test_pilot_is_populated_after_all_older_guards_and_measured_before_final(self):
        ...
        self.assertIn("for name in (*_ML_RUN_TABLES, *_ML_PILOT_TABLES, _RESULT_PASSAGE_TABLE, *_LEGACY_CORPUS_TABLES)", source)
    ...
    def test_run_contracts_are_checked_empty_and_populated_after_older_guards(self):
        ...
        self.assertIn("for name in (*_ML_RUN_TABLES, *_ML_PILOT_TABLES, _RESULT_PASSAGE_TABLE, *_LEGACY_CORPUS_TABLES)", functions["_assert_empty_ml_runs"])
        ...
        for marker in ("service.propose(session, **args)", "assert await state(session) == before",
                       'recorded(session, review, "decision")', 'decision="revoke"', "assert await state(session) == after"):
            self.assertIn(marker, functions["_ml_runs_on_migrated_schema"])

    def test_run_migration_has_no_seed_and_locks_before_history_preservation(self):
        source = (ROOT / "api/alembic/versions/0074_ml_use_runs.py").read_text()
        upgrade, downgrade = source.split("def upgrade()", 1)[1].split("def downgrade()", 1)
        self.assertNotIn("INSERT", upgrade)
        self.assertIn("relation.create(op.get_bind())", upgrade)
        self.assertLess(downgrade.index("IN ACCESS EXCLUSIVE MODE"), downgrade.index("SELECT EXISTS"))
        self.assertLess(downgrade.index("raise RuntimeError"), downgrade.index("relation.drop"))

    def test_ml_rights_are_checked_empty_and_rehearsed_after_existing_history_guards(self):
        source = (ROOT / "scripts/run_test_migrations.py").read_text()
        functions = {node.name: ast.get_source_segment(source, node) for node in ast.parse(source).body
                     if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef))}
        self.assertIn("public.{_ML_RIGHTS_TABLE}", functions["_assert_empty_ml_submissions"])
        markers = ("_ml_submissions_downgrade_guard(capability, engine, config, submission_id)",
                   "_ml_rights_roundtrip(capability, engine, config)",
                   "_ml_rights_on_migrated_schema(capability)",
                   "_ml_rights_roundtrip(capability, engine, config, populated=True)",
                   'recorder.phase(connection, "final")')
        positions = [functions["main"].index(marker) for marker in markers]
        self.assertEqual(positions, sorted(positions))
        roundtrip = functions["_ml_rights_roundtrip"]
        for marker in ('command.downgrade(config, "0072_ml_use_submissions")',
                       '"retained independent review history"', 'command.upgrade(config, "head")',
                       "assert snapshot(connection) == before", "to_regprocedure"):
            self.assertIn(marker, roundtrip)
        rehearsal = functions["_ml_rights_on_migrated_schema"]
        for marker in ('service.decide(session, **args)', 'recorded(session, args)', 'decision="revoke"',
                       "assert await state(session) == before", "assert await state(session) == after"):
            self.assertIn(marker, rehearsal)

    def test_ml_rights_migration_never_creates_permissions_and_refuses_audit_erasure(self):
        source = (ROOT / "api/alembic/versions/0073_ml_use_rights.py").read_text()
    ...
    def test_ml_role_roundtrip_preserves_all_prior_rows_and_refuses_old_head(self):
        ...
        self.assertIn('if name not in {"alembic_version", _ML_USE_ROLE_TABLE, *_ML_SUBMISSION_TABLES, _ML_RIGHTS_TABLE, *_ML_RUN_TABLES, *_ML_PILOT_TABLES, _RESULT_PASSAGE_TABLE, *_LEGACY_CORPUS_TABLES}', block)
    ...
    def test_main_barrier_roundtrip_retains_every_v1_byte_and_the_exact_frozen_function(self):
        ...
        for marker in ('if name not in {"alembic_version", _ML_USE_ROLE_TABLE, *_ML_SUBMISSION_TABLES, _ML_RIGHTS_TABLE, *_ML_RUN_TABLES, *_ML_PILOT_TABLES, _RESULT_PASSAGE_TABLE, *_LEGACY_CORPUS_TABLES}', "_assert_empty_discovery_projections(connection)",
            ...
            'command.upgrade(config, "head")', '"0081_index_search"', "_assert_empty_ml_use_roles(connection)",
